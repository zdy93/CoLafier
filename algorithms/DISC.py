...
class DISC:
    def __init__(
        ...
        if 'cifar' in self.dataset or 'tiny_imagenet' in self.dataset:
            if 'sym' not in config['noise_type'] and 'asym' not in config['noise_type']:
        ...
        if 'cifar' in config['dataset']:
            ...
            N = 65944
        elif 'fashionMNIST' in config['dataset']:
            N = 60000
        self.N = N
    ...
    def train(self, train_loader, epoch):
        ...
        if epoch < self.start_epoch:
            for (images, targets, indexes) in pbar:
                ...
                wandb.log({"loss_1": loss_sup, "epoch": epoch})
        else:
            for (images, targets, indexes) in pbar:
                w_imgs, s_imgs = Variable(images[0]).to(self.device, non_blocking=True), \
                                 Variable(images[1]).to(self.device, non_blocking=True)
    ...
    def evaluate(self, test_loader):
        ...
        log_dict = {key + "_model_1": value for key, value in eval_dict.items()}
        log_dict['epoch'] = self.tmp_epoch
        wandb.log(log_dict)
        return eval_dict
