...
from .randaugment import TransformFixMatch_CIFAR10, TransformFixMatch_CIFAR100, Transform_2strong_CIFAR100, \
    TransformGJS, Transform2Weak_CIFAR10, Transform2Weak_CIFAR100
...
class NoiseDataset(torchvision.datasets.VisionDataset):
    ...
    def __init__(
        ...
        self.ground_truth_targets = self.targets.copy()
    ...
    def load_noisy_label(self):
        ...
        if isinstance(noise_label, dict):
            ...
            self.targets = noise_label[self.noise_type].tolist()
        else:
...
class NoiseCIFAR10(CIFAR10, NoiseDataset):
    ...
    def __init__(
        ...
        if mode == 'train_index':
            ...
            self.transform_train = TransformFixMatch_CIFAR10(
                (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        elif mode == 'train_index_regular':
            self.transform_train = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize((0.4914, 0.4822, 0.4465),
                                     (0.2023, 0.1994, 0.2010)),
            ])
        elif mode == 'train_index_triple':
            self.transform_train = TransformFixMatch_CIFAR10(
                (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
    ...
    def __getitem__(self, index):
        ...
        elif self.mode == 'train_index_triple':
            ori = self.transform_ori(image)
            img = self.transform_train(image)
            return (img, ori), target, index
        elif self.mode == 'train_index_regular':
            img = self.transform_train(image)
            return img, target, index
        elif self.mode == 'test':
...
class NoiseCIFAR100(CIFAR100, NoiseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train_single':
            ...
            img2 = self.transform_train_strong(image)
            return raw, img1, img2, target
        elif self.mode == 'train_index' or self.mode == 'train_index_2strong' or self.mode == 'train_index_regular':
            img = self.transform_train(image)
            return img, target, index
        elif self.mode == 'train_index_triple':
            ori = self.transform_ori(image)
        ...
        if mode == 'train_single':
            if self.cifar_type == 'cifar-10':
                train_dataset = NoiseCIFAR10(root=self.root, train=True, transform=None, noise_type=self.noise_type,
                                             percent=self.percent, mode=mode, noise_path=self.noise_path)
            ...
            return train_loader
        elif mode == 'train_index' or mode == 'train_index_2strong' or mode == 'train_index_regular' or mode == 'train_index_triple':
            if self.cifar_type == 'cifar-10':
                train_dataset = NoiseCIFAR10(root=self.root, train=True, transform=None, noise_type=self.noise_type,
                                             percent=self.percent, mode=mode, noise_path=self.noise_path)
            elif self.cifar_type == 'cifar-100':
