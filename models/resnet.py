...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, weights=None, get_feat=False) -> Any:
        if weights == None:
...
class ResNetWithLabel(ResNet):
    def __init__(
        ...
        super().__init__(block, layers, input_channel, num_classes, zero_init_residual, groups, width_per_group,
    ...
    def forward(self, x: Tensor, label: Tensor, weights=None, get_feat=True, merge_concat=False, activation=None, label_2=None, lam=None, embedding_type="regular") -> Any:
        # See note [TorchScript super()]
...
def _resnet(
    ...
    if with_label:
        model = ResNetWithLabel(block, layers, input_channel=input_channel, num_classes=num_classes, **kwargs)
    else:
...
def resnet18(
    ...
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   input_channel=input_channel, num_classes=num_classes, with_label=with_label, **kwargs)
